...
import config
from engine.drawing import OrientationManager, plot_circuit, plot_sector
...
def draw_circuit_view(duration=None, matrix=None, canvas=None, orientation_mgr=None, sector_index=None):
    """Renders the circuit map (and optional sector highlight) to the matrix or dummy canvas for the specified duration (seconds)."""
    if duration is None:
        duration = config.DISPLAY_DURATION
    ...
    while time.time() - start_time < duration:
        ...
        plot_circuit(canvas, orientation_mgr)
        if sector_index is not None:
            plot_sector(canvas, orientation_mgr, n=sector_index)
